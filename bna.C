...
void bna(const char *inputFile, const char *outputFile)
{
  ...
  for(entry = 0; entry < numberOfEntries; ++entry)
  {
    ...
    for(int i=0; i<branchJet->GetEntries(); ++i){
      const Jet* jeti = (const Jet*) branchJet->At(i);
      if (jeti->BTag == 1) continue;
      Jet_pt[nJet] = jeti->PT;
      Jet_eta[nJet] = jeti->Eta;
      Jet_phi[nJet] = jeti->Phi;
      if (jeti->PT < 30 || abs(jeti->Eta) > 2.5) continue;
      ++nJet;
      if (nJet >= Jet_N ) break;
    }
    if (nJet < 4) continue;
    ...
    if( matchedbjets.size() > 1){ 
      ...
      cout << matched_mbb << endl;
      cout << matched_dRbb << endl;
